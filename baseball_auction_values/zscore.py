...
pitcher_data = {
    'Name': ['Pitcher1', 'Pitcher2', 'Pitcher3'],
    'W': [15, 12, 10],
    'SV': [0, 0, 30],
    'K': [200, 180, 160],
    'ERA': [3.20, 3.50, 3.80],
...
hitter_weights = {
    'HR': 1.2,    # Value home runs 20% more
    'R': 0.8,     # Value runs 20% less
    'RBI': 1.2,   # Value RBIs 20% more
    'SB': 1.5,    # Value steals 50% more
    'AVG': 1.0    # Normal weight for average
}

pitcher_weights = {
    'W': 0.8,     # Value wins 20% less
    'SV': 1.2,    # Value saves 20% more
    'K': 1.3,     # Value strikeouts 30% more
    'ERA': 1.0,   # Normal weight for ERA
    'WHIP': 1.0   # Normal weight for WHIP
}

hitters_df = pd.DataFrame(hitter_data)
pitchers_df = pd.DataFrame(pitcher_data)

valued_hitters, valued_pitchers = calculate_auction_values(
    hitters_df, 
    pitchers_df,
...
# Display results with category contributions
print("\nTop Hitters:")
print(valued_hitters[['Name', 'HR_contribution', 'R_contribution', 'RBI_contribution', 
                      'SB_contribution', 'AVG_contribution', 'dollar_value']].head())
print("\nTop Pitchers:")
print(valued_pitchers[['Name', 'W_contribution', 'SV_contribution', 'K_contribution', 
                       'ERA_contribution', 'WHIP_contribution', 'dollar_value']].head())
"""